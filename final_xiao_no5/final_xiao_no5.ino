...
{
  // XIAO 4 的消息到达（优先级最高）
  if (NowSerial_4.available() >= 0 && !tumbler_executed) {
    int bytesRead = NowSerial_4.readBytes(&need_num_xiao, 1);
    bytesRead += NowSerial_4.readBytes(&need_extra_leds, 1);
    if (bytesRead == 2) {
      Serial.print("Received array from XIAO 4: ");
  ...
  switch(taskflow) {
    ...
    case 2:                    // 不倒翁工作流
      if (!tumbler_executed) { // 如果不倒翁效果尚未执行过
        ...
        if (NowSerial_4.availableForWrite()) { // 告诉上一个XIAO我灯效结束了
          NowSerial_4.write(0);
          Serial.println("Sent tumbler done message to XIAO 4");
        }
