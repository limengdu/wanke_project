...
const MacAddress peer_mac_8({0x54, 0x32, 0x04, 0x33, 0x1F, 0xF8});

ESP_NOW_Serial_Class NowSerial_6(peer_mac_6, ESPNOW_WIFI_CHANNEL, ESPNOW_WIFI_IF);
ESP_NOW_Serial_Class NowSerial_8(peer_mac_8, ESPNOW_WIFI_CHANNEL, ESPNOW_WIFI_IF);
...
void loop()
{
  if (NowSerial_6.available()) {
    int receivedData = NowSerial_6.read();
    ...
    // 发送 ESP-NOW 消息给下一个设备
    if (NowSerial_8.availableForWrite()) {
        if (NowSerial_8.write(receivedData) <= 0) {
