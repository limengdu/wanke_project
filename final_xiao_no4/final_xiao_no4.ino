...
const MacAddress peer_mac_5({0x54, 0x32, 0x04, 0x33, 0x30, 0x94});

ESP_NOW_Serial_Class NowSerial_3(peer_mac_3, ESPNOW_WIFI_CHANNEL, ESPNOW_WIFI_IF);
ESP_NOW_Serial_Class NowSerial_5(peer_mac_5, ESPNOW_WIFI_CHANNEL, ESPNOW_WIFI_IF);
...
void setup()
{
  ...
  NowSerial_5.begin(115200);
  Serial.println("XIAO NUM 4 Ready.\n");
}
...
{
  // XIAO 3 的消息到达（优先级最高）
  if (NowSerial_3.available() >= 0 && !tumbler_executed) {
    int bytesRead = NowSerial_3.readBytes(&need_num_xiao, 1);
    bytesRead += NowSerial_3.readBytes(&need_extra_leds, 1);
    if (bytesRead == 2) {
      Serial.print("Received array from XIAO 3: ");
  ...
  // 不倒翁效果结束的消息传来
  int receivedData_5 = 1;
  if (NowSerial_5.available()) {
    receivedData_5 = NowSerial_5.read();
    Serial.print("Received data from XIAO 5: ");
    Serial.println(receivedData_5);
  }
  
  // 如果收到来自 XIAO 5 的消息,说明不倒翁效果已完成
  if (!receivedData_5) {
    tumbler_done = true; // 标记不倒翁效果已完成
    // 发送消息给XIAO 3,通知不倒翁效果已完成
    if (NowSerial_3.availableForWrite()) {
      NowSerial_3.write(0);
      Serial.println("Sent tumbler done message to XIAO 3");
    }
    ...
    case 2:                    // 不倒翁工作流
      if (!tumbler_executed) { // 如果不倒翁效果尚未执行过
        ...
        if (need_num_xiao > 0) {
          ...
          if (NowSerial_5.availableForWrite()) {
            int bytesWritten = NowSerial_5.write(need_num_xiao);
            bytesWritten += NowSerial_5.write(need_extra_leds);
            if (bytesWritten == 2) {
              Serial.println("Send data to XIAO 5 Successfully");
            } else {
              Serial.println("Failed to send data to XIAO 5");
            }
          }
          // 再点亮灯条
          ...
          if (NowSerial_5.availableForWrite()) {
            NowSerial_5.write(0);
            NowSerial_5.write(0);
            Serial.println("Send 0 to XIAO 5");
          }
          // 只亮本灯条的对应的灯珠数量
